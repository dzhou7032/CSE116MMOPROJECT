...
import DeadmanPlayground.Types.{World,Player}
...
class TestGame extends FunSuite {
  /*test("testing for winner") {
    ...
    val units: World = new World(Map(
      0 -> new Player (List(List(0,0), (List(1, 0), List(1,1))),
...
*/
  ...
  */
  ...
  test("testing testFire function") {
    ...
    assert(shot.hitScan(map1, direction1) == false, "failed testing")
    val direction2: Array[Array[Int]] = Array(
      Array(2, 2), Array(-1, 0))
    assert(shot.hitScan(map1, direction2) == true, "failed testing")
    val direction3: Array[Array[Int]] = Array(
      Array(2, 2), Array(0, -1))
    assert(shot.hitScan(map1, direction3) == true, "failed testing")
    val direction4: Array[Array[Int]] = Array(
      Array(0,0), Array(1, 0))
    assert(shot.hitScan(map1, direction4) == false, "failed testing")
  }
}
*/